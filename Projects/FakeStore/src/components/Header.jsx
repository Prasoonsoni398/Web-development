...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-slate-950/95 shadow-sm backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-6 text-sm font-medium text-slate-100 md:flex">
          ...
          <Link to="/product" className="transition hover:text-cyan-300">
            Product
          </Link>
      ...
      <div className={`md:hidden overflow-hidden bg-slate-950 text-slate-100 transition-all duration-300 ${menuOpen ? "max-h-72" : "max-h-0"}`}>
        <nav className="space-y-2 border-t border-slate-800 px-4 py-4">
          <Link to="/" onClick={() => setMenuOpen(false)} className="block rounded-2xl px-4 py-3 transition hover:bg-slate-900 hover:text-cyan-300">
            Home
          </Link>
          <Link to="/about" onClick={() => setMenuOpen(false)} className="block rounded-2xl px-4 py-3 transition hover:bg-slate-900 hover:text-cyan-300">
            About
          </Link>
          <Link to="/product" onClick={() => setMenuOpen(false)} className="block rounded-2xl px-4 py-3 transition hover:bg-slate-900 hover:text-cyan-300">
            Product
          </Link>
          <Link to="/contact" onClick={() => setMenuOpen(false)} className="block rounded-2xl px-4 py-3 transition hover:bg-slate-900 hover:text-cyan-300">
            Contact
          </Link>
        </nav>
